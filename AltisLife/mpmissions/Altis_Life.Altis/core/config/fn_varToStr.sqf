...
switch (_var) do
{
	...
	case "life_inv_mauer": {"Absperrung"};
	...
	case "life_inv_methu": {"Unverarbeitetes Meth"};
    	...
    	case "life_inv_bitcoinu": {"Festplatte"};
    	case "life_inv_bitcoinp": {"bitcoins"};
    	case "life_inv_bottledwhiskey": {"Whiskey (21 Jahre)"};
	case "life_inv_bottledbeer": {"Goasmass (bayr. Bierspezialität)"};
	case "life_inv_tentciv": {"Zelt"};
	case "life_inv_profiwerkzeug": {"profiwerkzeug"}; //profiwerkzeug
	...
	case "license_civ_ingi": {"Ingenieursausbildung"};

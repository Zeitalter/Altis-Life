...
//-- NEUER CODE fia de ka wos
	_medicsOnline = {_x != player && side _x == independent} count playableUnits;
	if(_medicsOnline > 0)then{
		life_respawn_timer = 5;        //nach so vui minutn ko a respawnen wenn da hans online is
	}else{
		life_respawn_timer = 2;      //wenn a em ned online is dann guit des
	};
//-- ENDE	

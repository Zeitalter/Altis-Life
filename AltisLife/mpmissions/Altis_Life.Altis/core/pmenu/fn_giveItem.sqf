...
if(!([_val] call TON_fnc_isnumber)) exitWith {hint "Du hast keine Zahl eingegeben!";ctrlShow[2002,true];};
if(parseNumber(_val) <= 0) exitWith {hint "Du hast keinen Wert angegeben den du Geben willst!";ctrlShow[2002,true];};
if(isNil "_unit") exitWith {ctrlShow[2001,true]; hint "Der gewählte Spieler ist nicht in Reichweite!";};
if(!([false,_item,(parseNumber _val)] call life_fnc_handleInv)) exitWith {hint "Du konntest die Menge nicht geben, vielleicht hast du nicht so viel?";ctrlShow[2002,true];};
